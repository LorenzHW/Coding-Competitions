...
EXPECTED_OUTPUT = ['Case #1: 1', 'Case #2: 0', 'Case #3: IMPOSSIBLE', 'Case #4: 2', 'Case #5: 0', 'Case #6: 5']
SHIELDS = ['1', '2', '1', '6', '2', '3']
PROGRAMS = ['CS', 'CS', 'SS', 'SCCSSC', 'CC', 'CSCSS']

NUM_TEST_CASES = len(EXPECTED_OUTPUT)
...
cmd = [sys.executable, "saving_universe.py"]
# cmd = sys.argv[1:]
...
for i in range(NUM_TEST_CASES):
  ...
  JudgePrint(p, SHIELDS[i] + " " + PROGRAMS[i])  # number of sections
  p.stdin.flush()
  ...
  if not test_case_passed:
    WrongAnswerExit(p, i + 1, user_input + " IS THE WRONG ANSWER")
